...
class MemoryRegion private constructor(
    ...
    companion object {
         ...
         * backed by a [io.karma.kmmio.MemoryRegion] to rea the file.
         *
         ...
         * backed by a [io.karma.kmmio.MemoryRegion] to write the file.
         *
         ...
         * @throws MemoryRegionException If the new mapping could not be created.
         * @throws IllegalArgumentException If the [size] is less than one byte.
         */
        fun map(
     ...
     * @throws IllegalArgumentException If this region was already disposed using [close]
     *  or if the [size] is less than one.
     */
    @OptIn(UnsafeNumber::class)
    fun resize(size: Long): Boolean {
        check(!isClosed) { "MemoryRegion has already been disposed" }
        require(size >= 1) { "Memory region size must be larger or equal to 1 byte" }
    ...
    override fun close() {
        check(!isClosed) { "Memory region has already been disposed" }
...
internal class MemoryRegionSource(
    ...
    override fun readAtMostTo(
        ...
        check(!region.isClosed) { "Memory region has already been disposed" }
        require(
...
internal class MemoryRegionSink(
    ...
    check(!region.isClosed) { "Memory region has already been disposed" }
    check(region.sync(SyncFlags.SYNC)) { "Memory region could not be synced: ${getLastError()}" }
}) {
    override fun write(
